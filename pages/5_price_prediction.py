...
second_latest_date = df["datetime"].nlargest(2).iloc[-1]
...
# Store selection using st.pills (multi-select, starts empty)
selected_stores = st.pills("Pick a Supermarket", supermarkets, selection_mode="multi")

# Only process if a store is selected
if selected_stores:
    ...
    # Ensure all subcategories appear, even if they have no price change
    ...
    # Group by Subcategory to ensure all are shown
    ...
    st.write("👆 **Please select at least one supermarket to see the price changes.**")
